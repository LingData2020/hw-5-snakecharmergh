...
```{r 1.2, include=FALSE}
fit1 <- lm(upsid$vowels ~ upsid$consonants)
summary(fit1)
```
...
ggplot(upsid, aes(x=consonants, y=vowels)) +
  geom_point(size=2) +
  geom_line(data = fortify(fit1), aes(x = upsid$consonants, y = .fitted), color = "blue") +
  theme_bw()
```
...
```{r 2.2, include=FALSE}
fit2 = lmer(vowels ~ consonants + (1|area), upsid)
summary(fit2)
```
...
ggplot(upsid, aes(x=consonants, y=vowels)) +
  ...
  geom_line(data = fortify(fit1), aes(x = upsid$consonants, y = .fitted), color = "gray") +
  geom_line(data = fortify(fit2), aes(x = upsid$consonants, y = .fitted, col=upsid$area)) +
  theme_bw()
```
...
```{r 2.4}
"Still significant. Slopes are flatter."
```
...
```{r 3.1, include=FALSE}
fit3 = lmer(vowels ~ consonants + (0 + consonants | area), upsid)
summary(fit3)
```
...
ggplot(upsid, aes(x=consonants, y=vowels)) +
  geom_point(size=2, color="gray") +
  geom_line(data = fortify(fit1), aes(x = upsid$consonants, y = .fitted), color = "gray") +
  geom_line(data = fortify(fit3), aes(x = upsid$consonants, y = .fitted, col=upsid$area)) +
  theme_bw()
```
...
```{r 3.3}
"Not significant. Slopes differ by area"
```
...
```{r 4.1, include=FALSE}
fit4 = lmer(vowels ~ 1 + consonants + (1 + consonants | area), upsid)
summary(fit4)
```
...
ggplot(upsid, aes(x=consonants, y=vowels)) +
  geom_point(size=2, color="gray") +
  geom_line(data = fortify(fit1), aes(x = upsid$consonants, y = .fitted), color = "gray") +
  geom_line(data = fortify(fit4), aes(x = upsid$consonants, y = .fitted, col=upsid$area)) +
  theme_bw()
```
...
```{r 4.3}
"No constants are not significant. Dependencies differ heavily by area"
```
