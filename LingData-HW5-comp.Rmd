...
```{r 1.1}
ggplot(shva, aes(x=f1, y=f2)) + geom_point()
```
...
```{r 1.2-a}
# f1 boxplot
ggplot(shva, aes(x=vowel, y=f1)) + geom_boxplot()
```

```{r 1.2-b}
# f2 boxplot
ggplot(shva, aes(x=vowel, y=f2)) + geom_boxplot()
```
...
```{r 1.4}
summarize(group_by(shva, vowel), COR=cor(f1,f2))
```
...
```
- No significant dependence for a.
- Significant dependence for A. Small slope, but significant.
- Positive correlation for y, but insignificant, because data are noisy.
```
...
```{r 3.1}
fisher.test(d_caus$Aux, d_caus$EPTrans)$p.value
```
...
```{r 3.3}
chisq.test(d_caus$Aux, d_caus$EPTrans)$expected
```
...
```{r 3.5}
chi.2=chisq.test(d_caus$Aux, d_caus$EPTrans)$statistic
sqrt(chi.2/nrow(d_caus))
```
...
Result:
```
We have found a signifiant association between
variables Aux and EPTrans (p < 0.001). The odds of "doen" were 2.6
times higher in group "Intr" than in group "Tr". Effext
size is medium (Cramer's V = 0.17)
```
...
```
It's called p-hacking and overestimates the significance.
```
...
```{r 3.9}
# We tested whether the use of the Dutch words "doen" and "laten" were correlated
# with any indicators of the gramatical context.
# We found that doen was used 2.6 times more often when used with an intransitive
# predicate.
```
.
